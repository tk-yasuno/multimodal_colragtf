...
class MultimodalHippoRAG2Retriever:
    """Multimodal HippoRAG2-style retriever with 4-axis fusion (v0.7.3)"""
    
    def __init__(self, 
                 embedding_model_name: str = "hotchpotch/static-embedding-japanese",
                 alpha_text: float = 0.4,
                 alpha_bm25: float = 0.3,  # v0.7.3: BM25 weight
                 alpha_triple: float = 0.2,
                 alpha_image: float = 0.1,
                 figure_boost: float = 1.2):
        self.alpha_text = alpha_text
        self.alpha_bm25 = alpha_bm25  # v0.7.3
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        Retrieve relevant blocks with 4-axis fusion (v0.7.3)
        
        Fusion formula:
            score = alpha_text * text_score + 
                   alpha_bm25 * bm25_score +  # v0.7.3
        ...
        # 5. Score fusion (4-axis)
        final_scores = (
            self.alpha_text * text_scores +
            self.alpha_bm25 * bm25_scores +  # v0.7.3
        ...
        for idx in top_indices:
            ...
            results.append({
                ...
                "bm25_score": float(bm25_scores[idx]),  # v0.7.3
...
def run_integrated_demo(image_path: Path):
    ...
    with 4-axis fusion (v0.7.3)
    """
    print("=" * 70)
    print("Integrated Demo: Image-based Disaster Lesson Extraction (v0.7.3)")
    print("4-axis Fusion: Text + BM25 + Triple + Image")
    print("=" * 70)
    print()
    ...
    retriever._load_bm25_index(data_dir / "bm25_index.pkl")  # v0.7.3
    ...
    # Step 2: Retrieval based on image understanding with 4-axis fusion
    print("🔍 Step 2: Retrieving Relevant Blocks (4-axis fusion)")
    print("-" * 70)
    print(f"Weights: text={retriever.alpha_text}, bm25={retriever.alpha_bm25}, " +
          f"triple={retriever.alpha_triple}, image={retriever.alpha_image}")
    print()
...
"""
    ...
    output = {
        "version": "v0.7.3",
        "fusion_method": "4-axis (Text + BM25 + Triple + Image)",
        "fusion_weights": {
    ...
    output_path = Path(__file__).parent / "indices" / f"demo_result_v073_{image_path.stem}.json"
