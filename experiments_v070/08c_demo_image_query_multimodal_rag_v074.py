...
"""
Integrated Demo: Image Understanding + Multimodal Retriever (v0.7.4)

Demonstrates the full pipeline with 4-axis fusion using Bayesian-optimized weights:
1. Input: Disaster image
2. Query: "この画像に移っている災害の教訓を整理したい。"
3. Image Understanding: llava:7b extracts disaster information
4. Retrieval: 4-axis fusion (Text + BM25 + Triple + Image) with optimized weights
5. Answer Generation: qwen2.5:7b synthesizes lessons learned

Version History:
- v0.7.2: 2-axis fusion (Text + Triple + Image)
- v0.7.3: 4-axis fusion (Text + BM25 + Triple + Image) with manual weights
- v0.7.4: 4-axis fusion with Bayesian-optimized weights (Optuna 50 trials)
          Optimized for: 0.5*Precision@5 + 0.2*Diversity + 0.3*AnswerQuality
          Best weights: α_text=0.2675, α_bm25=0.2903, α_triple=0.4422
"""
...
# Fix Windows PowerShell UTF-8 encoding issue
import io
...
class MultimodalHippoRAG2Retriever:
    """Multimodal HippoRAG2-style retriever with 4-axis fusion (v0.7.4)"""
    
    def __init__(self, 
                 embedding_model_name: str = "hotchpotch/static-embedding-japanese",
                 alpha_text: float = 0.2675,   # v0.7.4: Bayesian-optimized
                 alpha_bm25: float = 0.2903,   # v0.7.4: Bayesian-optimized
                 alpha_triple: float = 0.4422, # v0.7.4: Bayesian-optimized
                 alpha_image: float = 0.1,
    ...
    def _bm25_tokenize(text: str) -> List[str]:
        """Japanese bigram tokenization (character + 2-gram)"""
        chars = list(text)
    ...
    def retrieve(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        Retrieve relevant blocks with 4-axis fusion (v0.7.4)
        
        Fusion formula (Bayesian-optimized weights):
        ...
        where:
            alpha_text   = 0.2675 (optimized via Optuna, 50 trials)
            alpha_bm25   = 0.2903 (optimized via Optuna, 50 trials)
            alpha_triple = 0.4422 (optimized via Optuna, 50 trials)
            alpha_image  = 0.1000 (fixed)
        """
        # Encode query
        ...
        # 4. Image scores (placeholder - could be enhanced with visual features)
        image_scores = np.zeros(len(self.blocks))
        
        # 5. Score fusion (4-axis with optimized weights)
        final_scores = (
            self.alpha_text * text_scores +
            self.alpha_bm25 * bm25_scores +
            self.alpha_triple * triple_scores +
            self.alpha_image * image_scores
        )
...
def run_integrated_demo(image_path: Path):
    ...
    with 4-axis fusion using Bayesian-optimized weights (v0.7.4)
    """
    print("=" * 70)
    print("Integrated Demo: Image-based Disaster Lesson Extraction (v0.7.4)")
    print("4-axis Fusion: Text + BM25 + Triple + Image (Bayesian-optimized)")
    print("=" * 70)
    print()
    ...
    retriever = MultimodalHippoRAG2Retriever(
        alpha_text=0.2675,   # Optimized by Optuna (50 trials)
        alpha_bm25=0.2903,   # Optimized by Optuna (50 trials)
        alpha_triple=0.4422, # Optimized by Optuna (50 trials)
        alpha_image=0.1
    )
    ...
    retriever._load_bm25_index(data_dir / "bm25_index.pkl")
    ...
    # Step 2: Retrieval based on image understanding with optimized 4-axis fusion
    print("🔍 Step 2: Retrieving Relevant Blocks (Bayesian-optimized 4-axis fusion)")
    print("-" * 70)
    print(f"Optimized Weights (50 trials, TPESampler):")
    print(f"  text={retriever.alpha_text:.4f}, bm25={retriever.alpha_bm25:.4f}, " +
          f"triple={retriever.alpha_triple:.4f}, image={retriever.alpha_image:.4f}")
    print()
    ...
    for i, result in enumerate(results, 1):
        ...
        print(f"     ↳ text: {result['text_score']:.4f}, bm25: {result['bm25_score']:.4f}, " +
              f"triple: {result['triple_score']:.4f}, image: {result['image_score']:.4f}")
...
"""
    ...
    output = {
        "version": "v0.7.4",
        "fusion_method": "4-axis (Text + BM25 + Triple + Image) - Bayesian-optimized",
        "optimization_info": {
        ...
        "fusion_weights": {
            "alpha_text": retriever.alpha_text,
            "alpha_bm25": retriever.alpha_bm25,
            "alpha_triple": retriever.alpha_triple,
            "alpha_image": retriever.alpha_image
        },
    ...
    output_path = Path(__file__).parent / "indices" / f"demo_result_v074_{image_path.stem}.json"
